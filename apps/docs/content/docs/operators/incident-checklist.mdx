...
```bash
birch rotate <SECRET_NAME> --env prod --service <SERVICE> --redeploy
```

- [ ] **For multiple environments**, rotate in parallel:

```bash
birch rotate <SECRET_NAME> --env staging --service <SERVICE> &
birch rotate <SECRET_NAME> --env prod --service <SERVICE> --redeploy &
wait
```
...
```bash
birch audit <SECRET_NAME> --last 5
```
...
```bash
birch audit <SECRET_NAME> --env <ENV> --last 1
```
...
```bash
birch rotate <SECRET_NAME> --env <ENV> --service <SERVICE>
```
...
- [ ] **If repeated failures**:
  ...
  - Update Birch config/credentials
  - File bug report
...
```bash
birch rollback <SECRET_NAME> --env <ENV> --service <SERVICE> --redeploy
```
...
```bash
birch rotate <SECRET_NAME> --env dev
```

- [ ] **Identify fix**:
  - Application code change needed?
  - Birch configuration issue?
...
**Trigger**: Birch daemon stops responding or crashes
...
```bash
tail -100 ~/.birch/daemon.log
```

Or if using systemd:

```bash
sudo journalctl -u birch -n 100
```
...
```bash
birch config show | grep audit_log_path
```
...
```bash
rsync backup-server:/birch-audit-logs/ ~/.birch/logs/
```
...
```bash
crontab -e
# Add: 0 2 * * * rsync -av ~/.birch/logs/ backup-server:/birch-audit-logs/
```
