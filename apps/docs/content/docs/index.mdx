---
title: Birch
description: Peel. Rotate. Renew.
---

import { Card, Cards } from 'fumadocs-ui/components/card';

Birch is an open-source CLI tool for safe, fast secret rotation. Updates local `.env` files and production host secrets without proxying traffic. Applications call provider APIs directly with their own keys.

## Features

- **Key Pools**: Pre-configure multiple API keys for automatic sequential rotation on rate limits
- **Dev Mode**: Update `.env` files atomically with rollback support
- **Production Mode**: Integrate with major hosting providers (Vercel, Netlify, Render, Cloudflare, Fly.io) and cloud secret managers (AWS, GCP, Azure)
- **App-Signal Rotation**: Accept rotation requests from applications (e.g., on 429/rate-limit headers)
- **Manual & Scheduled**: Operator-triggered and cron-friendly
- **Rollback**: Time-boxed rollback with automatic key revocation
- **Audit Logging**: Cryptographically signed logs with Ed25519
- **Safety Rails**: Single-writer locks, cooldowns, dry-run mode, maintenance windows

## Quick Start

<Cards>
  <Card title="Installation" href="/docs/installation" />
  <Card title="Quick Start Guide" href="/docs/quick-start" />
  <Card title="SDK" href="/docs/sdk" />
  <Card title="Configuration" href="/docs/configuration" />
  <Card title="Usage Guide" href="/docs/usage" />
</Cards>

## Architecture

Birch is a single Rust binary with the following components:
- CLI for manual operations
- Optional background daemon for application-triggered rotations
- File-based locking and audit logging
- Connector architecture for provider integrations

No traffic proxying. No centralized secret storage. Direct updates to `.env` files and provider APIs.

## Next Steps

Choose your path based on your use case:

<Cards>
  <Card title="SDK Documentation" href="/docs/sdk" />
  <Card title="Developer Guide" href="/docs/usage" />
  <Card title="Operator Runbook" href="/docs/operators/runbook" />
  <Card title="Connectors" href="/docs/connectors" />
  <Card title="CLI Reference" href="/docs/cli-reference" />
</Cards>
