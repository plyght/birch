...
```bash
birch provider set github --mode oauth
```
...
```bash
birch provider set aws --mode kms
```
...
```bash
birch provider set custom --mode api_key
```
...
API documentation will be available once the service is deployed.
...
2. Log in to Birch SaaS: `birch login`
3. Create a workspace: `birch workspace create "My Team"`
