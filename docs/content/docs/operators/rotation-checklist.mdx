...
```bash
birch rotate <SECRET_NAME> --env prod --service <SERVICE> --dry-run
```
...
```bash
birch rotate <SECRET_NAME> --env prod --service <SERVICE>
```
...
```bash
birch audit <SECRET_NAME> --env prod --last 1
```
...
- [ ] Note rollback file location: `.birch-rollback`
...
```bash
birch rollback <SECRET_NAME> --env dev
```

Or manually:

```bash
cp .birch-rollback .env
```
...
```bash
birch audit <SECRET_NAME> --env <ENV> --last 5
```
